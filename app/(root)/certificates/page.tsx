    ...
    <div className="w-full z-20 flex-col px-4 relative flex justify-start items-center">
      <div className="max-w-[800px] pt-28 relative w-full flex flex-col gap-8 justify-center items-center">
        <div className="grid w-full relative grid-cols-1 md:grid-cols-2 gap-4">
          <div className="absolute z-22 bottom-0 left-0 w-full h-[500px] bg-linear-to-t from-background to-transparent"></div>
          {certificates &&
            certificates.data &&
            certificates.data.results.map((item: NotionPage) => (
              <div key={item.id} className="aspect-16/12 flex relative group overflow-hidden border border-primary/5 p-4">
                <Image
                  onError={() => certificates.refetch()}
                  alt="certificate"
                  width={400}
                  height={200}
                  // @ts-expect-error
                  src={item.properties.preview.files[0].file.url}
                  loading="lazy"
                  className="w-[140%] group-hover:translate-x-[5%] grayscale z-10 transition duration-500 group-hover:grayscale-0 translate-x-[10%] object-contain h-[120%]"
                />
