...
export default function Footer({ user }: { user: NotionPageResponse | null }) {
  return (
    <footer className="sticky bottom-0 bg-primary flex justify-center items-center px-4">
      <div className="max-w-[1200px] bg-primary rounded-2xl md:py-16 py-10 relative w-full flex flex-col gap-2 justify-center items-center">
        <div className="flex gap-8 border-b border-muted-foreground/20 pb-10 w-full flex-col md:flex-row justify-start items-start md:justify-between md:items-start">
          ...
          <div className="flex gap-1 justify-start flex-col items-start md:items-end">
            {navList.map((item: NavType, index: number) => (
              <a key={index} className="text-background tracking-[-1px] font-semibold text-lg" href={item.href}>
                {item.name}
              </a>
            ))}
          </div>
